...
tf.app.flags.DEFINE_string('pretrained_model_path',"./east_resnet_v1_50_rbox/resnet_v1_50.ckpt" , '')
...
def plot_model_history(ml,tl):
    ...
    axs[1].plot(range(1,len(tl)+1),tl)
    axs[1].plot(range(1,len(ml)+1),ml)
...
def main(argv=None):
    ...
    # add summary
    summary_writer = tf.summary.FileWriter("./east_resnet_v1_50_rbox/events.out.tfevents.e851621451e8", tf.get_default_graph())
    ...
    with tf.Session(config=tf.ConfigProto(allow_soft_placement=True)) as sess:
        ...
        summary_writer = tf.summary.FileWriter(FLAGS.checkpoint_path, sess.graph)
        ...
        for step in range(FLAGS.max_steps):
            ...
            if step % FLAGS.save_checkpoint_steps == 0:
                ...
                saver.save(sess, "./east_resnet_v1_50_rbox/model.ckpt",global_step=global_step)
